...
result="$eval_dir/Result/HiGHS/thread_$thread"
cutoff="300"
timeout="600"
...
for cut in $cutoff; do
  for ((i = 0; i < ${#all_datas[*]}; i++)); do
    ...
    for dir_file in $(cat $benchmark_list); do
      ...
      read -u 6
      {
        timeout $timeout $eval_dir/Evaluation/pre-compiled/HiGHS/highs $instance/$file --options_file $thread.opt --time_limit $cut
        echo >&6
