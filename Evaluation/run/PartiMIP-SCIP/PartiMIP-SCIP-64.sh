...
result="$eval_dir/Result/PartiMIP-SCIP/thread_$thread"
...
for cut in $cutoff; do
  for ((i = 0; i < ${#all_datas[*]}; i++)); do
    ...
    for dir_file in $(cat $benchmark_list); do
      ...
      read -u 6
      {
        timeout $timeout $eval_dir/Evaluation/pre-compiled/PartiMIP-SCIP/PartiMIP-SCIP -i $instance/$file -t $thread -c $cut
        echo >&6
